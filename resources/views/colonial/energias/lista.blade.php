            ...
            <div class="panel panel-white">
                <div class="panel-heading" style="height: auto">
                    <div style="display: flex; gap: 1em; justify-content: flex-end">
                        <form action="{{ route('energia-listar') }}" method="GET">
                        ...
                        <div>
                            @if (auth()->user()->isPermissao('energia', 'criar'))
                                <a href="{{ route('energia-criar') }}" class="btn btn-danger">
                ...
                <div class="panel-body">
                    <table class="table table-striped" style="margin-bottom: 0">
                        ...
                        <tbody>
                            @foreach ($energia as $linha) 
                                    <tr id="tr-energia-{{ $linha->id }}">
                                        <th>{{ $linha->id }}</th> 
                                        <td class="text-center">{{  date( 'd/m/Y' , strtotime( $linha->dt_consumo ) ) }}</td> 
                                        <td class="text-right">{{ number_format($linha->qtde,2,",",".") }}</td> 
                                        <td class="text-center"> 
                                                <div class="btn-group">
                                                    @if (auth()->user()->isPermissao('energia', 'criar'))
                                                        <a href="{{ route('energia-editar', $linha) }}"
                                                            class="btn btn-success btn-xs">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                    @endif

                                                    @if (auth()->user()->isPermissao('energia', 'excluir'))
                                                        <button class="btn btn-danger btn-xs" onclick="excluirCad('#tr-energia-{{ $linha->id }}', {{ $linha->id }})">
            ...
            function excluirCad(el, id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.get(`/colonial/energia-delete/${id}`)
                            .then((res) => {
