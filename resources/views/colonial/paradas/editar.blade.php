<x-layout.colonial.layout>
    ...
    <div id="main-wrapper" x-data="app">
        <div class="col-md-12 ">
            <form action="{{ route('parada-update', $parada) }}" method="POST" class="panel panel-white">
                @csrf
                            ...
                            <div class="form-group @if($errors->has('ordem')) has-error @endif ">
                                ...
                                <select class="form-control" name="ordem"  >
                                    ...
                                    @foreach ($ordem as $valor )
                                        <option value="{{ $valor->DocEntry }}" @if(old('ordem',$parada->cd_ordem)== $valor->DocEntry) selected  @endif >{!!   $valor->DocEntry . ' - '. date( 'd/m/Y' , strtotime( $valor->DueDate ) ) .  ' [ '.$valor->ProdName.' ] ' !!}</option> 
                                    @endforeach
                                </select>
                                @if($errors->has('ordem'))
                            ...
                            <div class="form-group @if($errors->has('tipo')) has-error @endif ">
                                ...
                                <select class="form-control" name="tipo"  >
                                    ...
                                    @foreach ($tipo as $valor )
                                        <option value="{{ $valor->cd_tipo }}" @if(old('tipo',$parada->cd_parada)== $valor->cd_tipo) selected  @endif >{!!   $valor->nm_tipo   !!}</option> 
                                    @endforeach
                                </select>
                                @if($errors->has('tipo'))
